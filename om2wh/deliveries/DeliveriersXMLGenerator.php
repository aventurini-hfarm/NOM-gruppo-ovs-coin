...
require_once "DeliveryObject.php";

class DeliveriersXMLGenerator extends FileGenerator {
    ...
    public function generatePickListFile() {
    ...
    private function generatePickList(DeliveryObject $delivery_object){
        ...
        array_push($xml_lines, '<bill_to_cust_first_name>'.$delivery_object->bill_to_info->getFirstname().'</bill_to_cust_first_name>');
        array_push($xml_lines, '<bill_to_cust_last_name>'.$delivery_object->bill_to_info->getLastname().'</bill_to_cust_last_name>');
        array_push($xml_lines, '<bill_to_address1>'.$delivery_object->bill_to_info->getStreet(1).'</bill_to_address1>');
        ...
        array_push($xml_lines, '<bill_to_email>'.$delivery_object->customer->email.'</bill_to_email>');
        array_push($xml_lines, '</bill_to_info>');

        //SHIPPING INFO
        array_push($xml_lines, '<ship_to_info>');
        array_push($xml_lines, '<ship_to_cust_number>'.$delivery_object->customer->customer_no.'</ship_to_cust_number>');
        array_push($xml_lines, '<ship_to_cust_first_name>'.$delivery_object->ship_to_info->getFirstname().'</ship_to_cust_first_name>');
        array_push($xml_lines, '<ship_to_cust_last_name>'.$delivery_object->ship_to_info->getLastname().'</ship_to_cust_last_name>');
        array_push($xml_lines, '<ship_to_address1>'.$delivery_object->ship_to_info->getStreet(1).'</ship_to_address1>');
        array_push($xml_lines, '<ship_to_address2></ship_to_address2>');
        ...
        array_push($xml_lines, '<ascensore>'.($delivery_object->ship_to_info->getData('assemblydeliverylift') ? 'true':'false').'</ascensore>');
