...
class DeliveryExportDMManuale {
    ...
    public function export($lista_ordini) {
        ...
        foreach ($lista_ordini as $record) {
            ...
            $this->log->LogInfo("\nExport: ".$increment_id);
            $con = OMDBManager::getConnection();
            $tmp_order_number = ltrim($order->getDwOrderNumber(), '0');
            //$sql = "UPDATE delivery_export SET status=3 WHERE dw_order_number=".$tmp_order_number;
            //mysql_query($sql);
...
$deliveryManager = new DeliveryExportDMManuale();
