...
class ConfirmOrder {
    ...
    private function process($lista_ordini) {
        ...
        foreach ($lista_ordini as $increment_id) {
            $order = Mage::getModel('sales/order')->load($increment_id, 'increment_id');
            $order_customer_locale=$order->getCustomerLocale();
            $infoFiscale = $this->getInfoFiscale($increment_id, $order_customer_locale);

            if ($order->getData('needInvoice')=='true' || $infoFiscale->sopra_soglia=='1')
                $this->generaFattura($increment_id, $order_customer_locale);
            else
            ...
            //$this->removeConfermaOrdine($increment_id); // RINO 23/09/2016 todo rimuovere rem
        }
    }
    ...
    private function generaScontrino($increment_id, $order_customer_locale) {
        ...
        */
        ...
        echo "\nGenerato scontrino: ".$invoicepath;
        return $response;
    ...
    private function inviaEmailConfermaOrdine($increment_id, $order_customer_locale) {
        ...
        echo "\nEmail Address destinatario: ".$email_address;
    ...
    private function getListaConfermaOrdine() {
        ...
        $sql ="SELECT increment_id FROM conferma_ordine limit 50";
        //echo "\nLog: ".$sql;
        $res = mysql_query($sql);
        $lista = array();
        while ($row = mysql_fetch_object($res)) {
    ...
    public function getListaIncrementIdByDwOrderNumber($dw_order_number) {
        $con = OMDBManager::getMagentoConnection();
...
foreach ($tmp as $item) {
    echo "\nProcessing: ".$item;
    $increment_id = $t->getListaIncrementIdByDwOrderNumber($item);
    $lista[] = $increment_id;
}
print_r($lista);
$t->export($lista);
