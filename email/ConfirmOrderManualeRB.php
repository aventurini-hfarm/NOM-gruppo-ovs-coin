...
class ConfirmOrder {
    ...
    public function export($lista = null) {
        ...
        $lista_ordini = $this->getListaConfermaOrdine($lista);
    ...
    private function getInfoOrdine($increment_id) {
        ...
        $piva = $order_custom_attributes['partitaIva'] ? $order_custom_attributes['partitaIva']: ""; // RINO 8/07/2016 TODO generalizzare PIVA <PAESE><numero> es: ES02658765890
        ...
        $infoOrdine->iva = number_format(round($order->getBaseTaxAmount(),2),2);    // RINO 30/07/2016
        ...
        /*if ($country_id!='IT') {  //RINO 8/7/2016 il tracking proviene sempre da shipment
    ...
    private function getOrderLines($increment_id, $country) {  //RINO 09/09/2016
        ...
        foreach ($lines as $line) {
            ...
            $qty = $line['order_quantity'];
            $unit_price =  $line['unit_price'];
            $discount_value = $line['original_discount'];
            if (strtolower($country)=='es') $iva=1.21; else $iva=1.22;
            $total_discount = $total_discount + $line['discount_value_not_fmt'] - number_format($line['original_discount'] / $iva , 2) ;

            $total = ($qty * ($unit_price * $iva)) - $discount_value;
            ...
            $obj->qty = $qty;
            $obj->unit_price = $unit_price;
            $obj->unit_discount_price = $unit_price - $discount_value/$iva;
            $obj->discount_value = $discount_value;
            $obj->total = $total;
            $items[] = $obj;
        }
        ...
        foreach ($lista_sconti as $sconto) {
            ...
            $obj->qty = 1;
            $obj->unit_price = $total_discount  * -1;
            $obj->unit_discount_price = $obj->unit_price * -1;
            $obj->discount_value = 0;
            $obj->total = $total_discount * $iva * -1;
            $items[] = $obj;
    ...
    private function process($lista_ordini) {
        ...
        foreach ($lista_ordini as $increment_id) {
            ...
            if ($order->getData('needInvoice')=='true' || $infoFiscale->sopra_soglia=='1')
                $this->generaFattura($increment_id);
            else
                $this->generaScontrino($increment_id, $order_customer_locale);



            $this->inviaEmailConfermaOrdine($increment_id, $order_customer_locale ); // Rino 05/07/ aggiunto multilingua
            //$this->removeConfermaOrdine($increment_id);
        }
    }

    private function generaScontrino($increment_id, $order_customer_locale) {
        ...
        $infoFiscale = $this->getInfoFiscale($increment_id);  // RINO 04/09/2016
    ...
    private function inviaEmailConfermaOrdine($increment_id, $order_customer_locale) {
        ...
        $email_address = "rinobilla@tiscalinet.it";   // RINO 22/07/2016  RIMUOVE dopo i test
        $mail = new PHPMailer;
        $mail->CharSet = "UTF-8";
        ...
        switch($ocl) {  // Rino 07/08/2016 oggetto mail per lingua d'ordine
            case 'it': $mail->Subject = 'Conferma Spedizione OVS #'.$info_ordine->ordine; break;
            case 'en': $mail->Subject = 'Shipping Confirmation OVS #'.$info_ordine->ordine; break;
            case 'es': $mail->Subject = 'Confirmación de envío OVS #'.$info_ordine->ordine; break;
        }
        ...
        if ($info_ordine->need_invoice=='true' || $infoFiscale->sopra_soglia=='1') {
            ...
            $mail->addBCC('marco.barpi@everis.com');

            if (strtolower($infoFiscale->country) == 'es') {
                //$mail->addBCC('marco.barpi@gmail.com');
            }
    ...
    private function getListaConfermaOrdine($lista) {

        $con = OMDBManager::getMagentoConnection();

        $lista_str="'".implode("','",$lista)."'";
        $sql ="SELECT increment_id FROM sales_flat_order where dw_order_number in (".$lista_str.")";
        //echo "\nLog: ".$sql;
        $res = mysql_query($sql);
        $lista = array();
...
$lista=['00271387','00271408','00271522','00271543','00271559','00271830','00271950','00271968','00271980','00272028','00272154','00272323','00272338','00272358','00272431','00272486','00272535','00272599','00272861'];
