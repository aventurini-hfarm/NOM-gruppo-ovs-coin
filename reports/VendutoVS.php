...
class BillExport {
    ...
    private function getRecordSpedizioni($order) {
        ...
        $shippingAmount = $order->getShippingAmount() - $order->getBaseShippingDiscountAmount();
        $shippingDiscount = $promoObj[0]->value;
       ...
       // echo "\nShipping Discount: ".$shippingDiscount;
        ...
        if ($order->getShippingAmount() == 0  && $shippingDiscount != 0) {
            $imponibile =  (-1 * $shippingDiscount );
            $amount = 0;
            $iva = 0;
            $record->prezzo_cliente = $imponibile;
            $record->totale_netto = 0;
         //   echo "\nSono qui";
        } else {
            $lordo = $shippingAmount - $shippingDiscount;
            $imponibile = round($lordo/ $country_iva,2);
            $amount = $lordo;
            $record->prezzo_cliente = $lordo;
            $iva = $amount - $imponibile;
            $record->totale_netto = $imponibile;
        }

        //echo "\nCountryIva: ".$country_iva;
    ...
    private function getRecordSpedizioniRino($order) {
        //ITEM_FEE Spedizione
        $orderDBHelper = new OrderDBHelper($order->getDwOrderNumber());
        $order_custom_attributes = $orderDBHelper->getCustomAttributes();
    ...
    private function getCustomerInfoRecord($order) {
       ...
       // $record->nome_cognome = $customer->getName();  // RINO 22/09/2016 commentato da VS il 27/10/2016
        $record->nome_cognome = $order->getData('customer_name'); //27-10-2016
    ...
    private function getOrderLinesRecord($order) {
        ...
        foreach ($lines as $line) {
            ...
            $imponibile_tmp = $row_amount/$country_iva;
       ...
       // echo "|NFINE LINES";
        ...
        foreach ($lines as $line) {
           ...
           // print_r($line);
            ...
            $totale = ($qty * $line->base_price) +  $line->discount_value;
            if ($line->discount_value>0)
                $totale = ($qty * $line->base_price) -  $line->discount_value;

            //$totale = ($qty * $line->base_price) -  $line->discount_value;
            //echo "\nImponibile 2: ".$imponibile2;
            $imponibile_amount = -1 * round($totale/$country_iva,2);


            $iva = (-1*$totale) - $imponibile_amount;


            $record->totale_netto = -1 * $imponibile_amount;
            $record->iva = -1 * $iva;
            $record->totale = $totale;
            echo "\nTotale: ".$totale.", order: ".$order->getDwOrderNumber(), ", imponibile: ".$imponibile_amount, ", iva: ".$iva;
    ...
    */
    ...
    private function generateReport($lista_ordini, $lista_creditmemo) {
        ...
        foreach ($lista_ordini as $increment_id) {
            ...
            $record->descrizione = '';
            $record->qta = '';
            $record->prezzo_cliente = $record_spedizioni->prezzo_cliente;
            ...
            if ($record->metodo_spedizione== "Contanti") $record->metodo_spedizione = "Contrassegno";
            $record->punti_resi = "";
            $record->punti_guadagnati = $record_spedizioni->punti_guadagnati;
            $record->punti_spesi = $record_spedizioni->punti_spesi;
            $record->spese_spedizione = $record_spedizioni->spese_spedizione;
