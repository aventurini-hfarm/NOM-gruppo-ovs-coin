...
class DailySalesExport {
    ...
    private function getRecordSpedizioni($order, $country) {
        ...
        */
        ...
        $imponibile = round($imponibile_tmp,2);
    ...
    private function _getCustomerInfoRecord($order) {
        $orderDBHelper = new OrderDBHelper($order->getDwOrderNumber());
        $order_custom_attributes = $orderDBHelper->getCustomAttributes();
        ...
        */
        ...
        $record->email = $order->getData('customer_email'); //27-10-2016
    ...
    public function getOrderInfoRecord($order) {
        ...
        $tmp_amount = str_replace(',','', ($order->getBaseGrandTotal() - $tmpRecordSpedizioni->totale));
        //echo "\nTMP: ".$tmp_amount;

        $row_amount = round($tmp_amount,2);
        ...
        */
        ...
        $country_iva= $this->getCountryIva($order);  //RINO 26/09/2016
        $imponibile = round($row_amount/$country_iva,2); //RINO 26/09/2016
        ...
        $row_amount = round($order->getBaseGrandTotal(),2);
        $record->totale = $row_amount;

        //print_r($order->getData());
        //print_r($record);
        //echo "\nRow amount: ".$row_amount." , ". $order->getDwOrderNumber();
    ...
    private function generateReport($lista_ordini, $lista_creditmemo) {
        ...
        foreach ($lista_ordini as $increment_id) {
            ...
            //$record_customer_info = $this->getCustomerInfoRecord($order); //27/27/2016
            ...
            $record->totale_merce = $record_order_info->totale_netto;
            $record->iva = $record_order_info->iva;
            $record->vendita = $record_order_info->totale;
        ...
        foreach ($lista_creditmemo as $obj) {
           ...
           // $record_customer_info = $this->getCustomerInfoRecord($order); //27/10/2016
            $record_order_info = $this->getOrderInfoRecord($order);
            $record_payment_info = $this->getPaymentInfoRecord($order);
            //$record_merchandize_promo = $this->getMerchandizePromo($order);
            //$record_discount_promo = $this->getDiscountRecord($order);
            ...
            */
            ...
            $record->totale_merce = -1 *$imponibile;
            $record->iva = -1 * $iva;


            $record->vendita =  -1 * $info_creditmemo->grand_total;
...
*/
            ...
            $cella = "C".$riga;
            $xfIndex = $objWorksheet->getCell('C12')->getXfIndex();
            ...
            $objWorksheet->getCell($cella)->setXfIndex($xfIndex);
            //$objWorksheet->getStyle($cella)->getNumberFormat()->setFormatCode('#,##0.00');   //TODO ORDINE 00282245 per cifre > 999
            //$objWorksheet->getStyle($cella)->getNumberFormat()->setFormatCode('#.##0,00');
            //print_r($record);
...
*/
            ...
            $cella = "P".$riga;
            $xfIndex = $objWorksheet->getCell('P12')->getXfIndex();
            $objWorksheet->getCell($cella)->setValue(' ');
            $objWorksheet->getCell($cella)->setXfIndex($xfIndex);
            $objWorksheet->getStyle($cella)->applyFromArray($styleArray);
        ...
        $file_name = "DAILY_SALES_".$timestamp_file.".xlsx";
        $directory = '/tmp';
...
*/
        ...
        */
        ...
        $this->inviaEmail($full_name, null);
...
for ($i=02; $i <=16; $i++) {
    ...
    $start_date="$data/11/2016";
    $end_date="$data/11/2016";
    echo "\nStart datE: ".$start_date." , end: ".$end_date;
