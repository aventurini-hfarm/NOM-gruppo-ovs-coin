...
require_once realpath(dirname(__FILE__))."/../../omdb/ISODBHelper.php";
...
class InvoiceExport {
    ...
    public function __construct()
    {
        ...
        $this->log = new KLogger('/var/log/nom/invoice_export_vs.log',KLogger::DEBUG);
    ...
    public function export($start, $end, $start_date_cm, $end_date_cm) {
        ...
        echo "\nDATE START: ".$start;
        echo "\nDATE END: ".$end;
        echo "\nDATE START CM: ".$start_date_cm;
        echo "\nDATE END CM: ".$end_date_cm;
    ...
    public function exportByList() {


        $lista_ordini = array('200000968');
        $lista_creditmemo = null;
    ...
    private function getInfoFiscale($increment_id)
    {
        ...
        $infoFiscale->codice_iva = $country_details->codice_iva;                         //RINO 22/09/2016
        $infoFiscale->tipo_documento_vendita = $country_details->tipo_documento_vendita; //RINO 22/09/2016
        $infoFiscale->tipo_documento_reso = $country_details->tipo_documento_reso;       //RINO 22/09/2016
        $infoFiscale->aliquota_iva = $country_details->aliquota_iva;                     //RINO 22/09/2016
        $infoFiscale->nazione = $country_details->nome;                                  //RINO 22/09/2016
        $infoFiscale->iva = $country_details->iva;
    ...
    private function generateInvoiceExport($lista_ordini,$lista_creditmemo) {
        ...
        foreach ($lista_ordini as $increment_id) {
            ...
            if ($order->getData('needInvoice')=='true' || $infoFiscale->sopra_soglia=='1') {
                ...
                //$registro_iva = $country_details->country_id == 'it' ? "0" : "E"; // RINO 9/07/2016

                $centro_costo = $country_details->codice_ente . '0 ';  //RINO 23/09/2016
                $registro_iva = $country_details->registro_iva;        //RINO 23/09/2016
                ...
                if ($payment_method_selected == 'cashondelivery') {
                    ...
                    //$newDate_trx_date = date("Ymd", strtotime($trx_date)); //Commentato Vincenzo 03092016
                    $newDate_trx_date = date_format(date_create_from_format('d/m/Y', $order->getData('invoice_date')), 'Ymd');  //Vincenzo 03092016
                }
                /* END FIX per CO*/

                //$numero_doc = str_pad(CountersHelper::getInvoiceNumber(date('Y')), 4,'0', STR_PAD_LEFT);
                //echo "\nORDER_ID: ". $order->getIncrementId()." - ".$order->getData('invoice_number')."\n";
                ...
                $piva = $order_custom_attributes['partitaIva'] ? $infoFiscale->country . $order_custom_attributes['partitaIva'] : ""; // RINO 22/09/2016
                ...
                // RINO 22/09/2016
                $codice_iva1 = $infoFiscale->codice_iva;
                $aliquota_iva1 = $infoFiscale->aliquota_iva;
                $nazione = $infoFiscale->nazione;
                $tipo_documento = $infoFiscale->tipo_documento_vendita;
                $aliquota_corretta = $infoFiscale->iva;
                /* // RINO 30/07/2016
                ...
                */
                $order_grand_total = $order->getBaseGrandTotal();
                //echo "\nOrderGrand: ".$order_grand_total;
                //echo "\nAliquota: ".$aliquota_corretta;
                $imponibile_calcolato = round($order->getBaseGrandTotal()/$aliquota_corretta, 2);
                //echo "\nImponibile calcolato: ".$imponibile_calcolato;
                $tax_calcolato = $order->getBaseGrandTotal() - $imponibile_calcolato;
                //echo "\nTax calcolato: ".$tax_calcolato;

                $imponibile_tmp = $imponibile_calcolato;

                $imposta_tmp = $tax_calcolato;
                ...
                $tipo_persona = ($order_custom_attributes['partitaIva'] != '' ? 'G' : 'F'); // RINO 22/09/2016
                $nazione_nascita = "                                   ";
                ...
                /**
                 * SEZIONE SPESOMETRO - 24102016
                 */
               // echo "\nTest Spesometro";
               // print_r($infoFiscale);
               // echo "\nOrdine ".$order->getDwOrderNumber();
               // echo "\nFattura: ".$order->getData('needInvoice');
               // echo "\nSopraSoglia: ".$infoFiscale->sopra_soglia;

                if ($order->getData('needInvoice')=='true' && $infoFiscale->sopra_soglia=='0') {
                   // echo "\nSpesometro:".$order->getDwOrderNumber();
                    print_r($order_custom_attributes);
                    $data_nascita_tmp = $order_custom_attributes['birthday'];
                    $data_nascita = str_replace("-", "", $data_nascita_tmp);
                    $tipo_persona = 'F';
                    //$codice_nazione_estera_iso_nascita = strtoupper($country_details->country_id);
                    $codice_univoco_nazione = $order_custom_attributes['birthplace'];
                   // echo "\ncodice_univoco_nazione: ".$codice_univoco_nazione;
                    $isoDetails = ISODBHelper::getISODetails($codice_univoco_nazione);
                    //print_r($isoDetails);
                    $nazione_nascita = substr(str_pad($isoDetails->nome, 35),0,35);
                    $codice_nazione_estera_unico_nascita = $isoDetails->codice_iso;
                    $codice_nazione_estera_iso_nascita = str_pad(strtoupper($isoDetails->country_id), 3);

                }


                //FINE SPESOMETRO - 24102016
        ...
        foreach ($lista_creditmemo as $obj) {
            ...
            //if (!($creditmemo_id == 147 || $creditmemo_id == 164 ) ) continue;
            //if (!($creditmemo_id == 124  ) ) continue;
            $info_creditmemo = CreditMemoHelper::getCreditMemoDetails($creditmemo_id);
            if (!$info_creditmemo->invoice_number) continue;

            $order = Mage::getModel('sales/order')->load($order_id);

            //print_r($order->getData());
            $infoFiscale = $this->getInfoFiscale($order->increment_id); //VINCENZO 02092016
            //echo "\nINCRE: ".$order->increment_id;
            $orderDBHelper = new OrderDBHelper($order->getDwOrderNumber());
            $order_custom_attributes = $orderDBHelper->getCustomAttributes();
            ...
            $newDate_trx_date = date_format(date_create_from_format('d/m/Y', $info_creditmemo->bill_date), 'Ymd'); // RINO 30/08/2016  sostituito con bill_date perchè invoice_date era sempre vuoto
            ...
            //$numero_doc = str_pad(ltrim($info_creditmemo->bill_number,'0'),7,'0',STR_PAD_LEFT);         // RINO 30/08/2016
            //$data_documento_fattura = $order->getData('invoice_date');
            ...
            /*
            if ($infoFiscale->country == 'ES') {   //RINO 31/07/2016
                $codice_iva1 = "150";
                $aliquota_iva1 = "21000";
                $fatt_iva="1.21";
                $nazione="SPAGNA";
            ...
            $aliquota_iva1 = $infoFiscale->aliquota_iva;
            $nazione = $infoFiscale->nazione;
            $tipo_documento = $infoFiscale->tipo_documento_reso;
            $fatt_iva = $infoFiscale->iva;
            // } RINO 22/09/2016
            ...
            //echo "\nCRE_BILL_NUM: ". $info_creditmemo->bill_number." , ".$numero_doc;
            $record['rag_sociale_cognome'] = $rag_sociale_cognome;
            $record['rag_sociale_nome'] = $rag_sociale_nome;
            ...
            $record['data_nascita'] = $data_nascita;
            //$record['codice_nazione_estera_iso'] = $infoFiscale->country;
            $record['codice_nazione_estera_iso'] = strtoupper($infoFiscale->country);
            ...
            //$record['rif_num_fattura']=$info_creditmemo->rif_invoice_number; //COMMENTATO DA VINCENZO 02/09/2016
            //$record['rif_data_fattura']=$info_creditmemo->rif_invoice_date; //COMMENTATO DA VINCENZO 02/09/2016

            $record['rif_num_fattura']=$order->invoice_number; // DA VINCENZO 02/09/2016

            $newDate_trx_date = date_format(date_create_from_format('d/m/Y', $order->invoice_date), 'Ymd'); // VINCENZO 02/09/2016
            $record['rif_data_fattura']=$newDate_trx_date; // DA VINCENZO 02/09/2016
    ...
    private function getListaOrdiniDaExportare($start = null, $end = null, $status='complete') {
        ...
        $sql ="SELECT increment_id FROM sales_flat_order WHERE bill_date = '$start'
         AND (status='$status' OR status='closed')"; //AND needInvoice='true'";
    ...
    private function getListaCreditMemoDaExportare($start = null, $end = null) {
        ...
        /* COMMENTATO VINCENZO 03092016
        //$t = new CreditMemoHelper();
        //$lista = $t->getListaCreditMemoExportare($start, $end);
        */
        $lista = $this->getListaCreditMemoExportareLocale($start, $end);

        return $lista;
    }


    public function getListaCreditMemoExportareLocale($start = null, $end = null) {

        $con = OMDBManager::getMagentoConnection();

        //$sql ="SELECT order_id, entity_id FROM sales_flat_creditmemo WHERE (created_at BETWEEN '$start' AND '$end')";
        $sql ="SELECT order_id, entity_id FROM sales_flat_creditmemo WHERE invoice_date = '$start'";
...
$start_date_cm = "2016-10-13 00:00:00";
$end_date_cm = "2016-10-13 23:59:59";


$current_date= "2016-10-13";
$tmp_end_date = "2016-11-08";
while ($current_date != $tmp_end_date) {
    echo "\nCurrent_date: ".$current_date;
    $start_date = date('d/m/Y', strtotime($current_date));
    $end_date =  date('d/m/Y', strtotime($current_date));
    $start_date_cm =  date('d/m/Y', strtotime($current_date))." 00:00:00";
    $end_date_cm =  date('d/m/Y', strtotime($current_date)) ." 23:59:59";
    echo "\nSDATE:".$start_date.", ".$end_date.", ".$start_date_cm.", ".$end_date_cm;
    $current_date = date('Y-m-d', strtotime('+1 day', strtotime($current_date)));
    $t->export($start_date, $end_date, $start_date, $end_date );
    sleep(10);
