...
class BillExport {
    ...
    public function exportFlusso($bill_numbers) { // RINO 9/7/2016

        $this->log->LogInfo("Start");
        $lista_ordini = $this->getListaOrdiniByScontrino( $bill_numbers );
    ...
    private function generateBillExport($ente, $lista_ordini, $lista_creditmemo, $sopra_soglia) {
        ...
        foreach ($lista_ordini as $increment_id) {
            ...
            if ($orderCountry->codice_ente == $codice_ente) {  // rino 19/09/2016
                ...
                //$trx_date = date_format(date_create_from_format('d/m/Y', $order->getData('bill_date')), 'Y-m-d'); //solo per ritrasmettere forzatamente quelli del 2/11
                ...
                //$newDate_trx_date = "01/11/2016 10:00:00"; //SOLO PER IL CASO SPECIFICO ; DA RIMUOVERE IN PRODUZIONE
                ...
                if (round($totale_controllo,2) != round($order->getGrandTotal(),2)) {
                    echo "\nAttenzione squadratura ordine: ".$order->getDwOrderNumber()." totale_controllo=".$totale_controllo." , totale_ordine:".$order->getGrandTotal()."\n";
        ...
        foreach ($lista_creditmemo as $obj) {
            ...
            if ($orderCountry->codice_ente == $codice_ente) {  //RINO 19/09/2016
                ...
                if ($discount_value) {
                    ...
                    //echo "\nTotale_controllo discount: ".$totale_controllo;
                }
                ...
                if ($shipping_amount>0)
                    array_push($tmp_lista, $trx_discount); //spese aggiunte solo se > 0
               ...
               // array_push($lista_record, $txr_tender);
                ...
                if (round($totale_controllo,2) != round($crdetails->grand_total,2)) {
                    echo "\nAttenzione squadratura resi ($creditmemo_id)- ($trx_header_id): totale_controllo=".$totale_controllo." , totale_ordine:".$crdetails->grand_total."\n";
                    //foreach ($tmp_lista as $item)
                    //    print_r ($item);
                    foreach ($tmp_lista as $item)
                        array_push($lista_record, $item);
    ...
    private function getListaOrdiniByScontrino($bill_numbers) {
        ...
        foreach ($bill_numbers as $bill_number) {
            //$sql ="SELECT increment_id FROM sales_flat_order WHERE bill_number='$bill_number' AND (status='complete' OR status='closed')";
            $sql ="SELECT increment_id FROM sales_flat_order WHERE bill_number='$bill_number'";
            ...
            while ($row = mysql_fetch_object($res)) {
                $this->log->LogDebug("Record trovati:".$row->increment_id);
                $lista[] = $row->increment_id;
                echo "\nFound: ".$bill_number." , ".$row->increment_id;
                $found = true;
            }
...
$lista_scontrini=array("191318");
$t->exportFlusso($lista_scontrini);
...
/*
$start_date="24/10/2016";
$end_date="24/10/2016";
$start_date_cm = "24/10/2016";
$end_date_cm = "24/10/2016";
*/
