...
class OrderXMLAnalyzer {
    ...
    private function getItems($lista_nodi) {
        ...
        foreach ($lista_nodi as $nodo) {
            $xml = $nodo;
            $item = new ItemObject();
            $item->net_price = (string)$xml->{'net-price'};         // prezzo netto unitario
            $item->tax = (string)$xml->{'tax'};
            $item->gross_price = (string)$xml->{'gross-price'};     // prezzo lordo totale
            $item->base_price = (string)$xml->{'base-price'};       // prezzo netto unitario
            ...
            $item->discount_gross_price = (String)$tmpObj->{'gross-price'};     // RINO 02/03/2016
            $item->discount_net_price = (String)$tmpObj->{'net-price'};         // RINO 02/03/2016
            $item->original_discount = (String)$tmpObj->{'gross-price'};        // RINO 03/03/2016
    ...
    private function getShipping(SimpleXMLElement $xmlContent = null) {
        ...
        $item->discount_net_price = (String)$tmpObj->{'net-price'};
    ...
    private function getTotalsRino(SimpleXMLElement $xmlContent = null) {

        $xml = $xmlContent;
        $item = new TotalsObject();

        $obj = $xml->xpath("merchandize-total")[0];                                         //RINO 01/08/2016
        $values = array( 'net-price'=>(string)$obj->{'net-price'},                          //RINO 01/08/2016
            'tax'=>(string)$obj->{'tax'}, 'gross-price'=>(string)$obj->{'gross-price'});    //RINO 01/08/2016
        $item->merchandize_total = $values;                                                 //RINO 01/08/2016
        ...
        foreach ($tmpObjs as $tmpObj) {
            ...
            $values = array( 'net-price'=>(string)$obj->{'net-price'},
                ...
                'discount-net-price'=>(String)$tmpObj->{'net-price'},           // RINO 01/08/2016
                'discount-tax'=>(String)$tmpObj->{'tax'});                      // RINO 01/08/2016


            $lista_valori[] = $values;

        }
        //$item->merchandize_total = $lista_valori;                                         //RINO 01/08/2016
        $item->merchandize_total_price_adjustments_price_adjustment = $lista_valori;        //RINO 01/08/2016
    ...
    private function getPayment(SimpleXMLElement $xmlContent = null) {
        ...
        if ($custom_method) {
            ...
            if ($method_name=="CHIOSCO") {
                $paymentObj->type="CHIOSCO";
                $paymentObj->amount = (string)$xml->{'amount'};
                return $paymentObj;
            }
        }
    ...
    public function  processOrderSection(SimpleXMLElement $xmlContent = null)
    {
        ...
        if ($order->order_no!="281584") return;
        ...
        $order->isAuthenticated = "true"; //default assumo che siano utenti authenticati
        ...
        foreach ($customFields as $key=>$value) {
            ...
            if ('isAuthenticated'==$key) {//per gestione guest checkout 27/10/2016
                $order->isAuthenticated = (string)$value;
            }
        ...
        //aggiunge ordine su Magento se non esiste già // RINO 05/08/2016
        $check_order = Mage::getModel('sales/order')->load($order->order_no, 'dw_order_number');

        if ($check_order->getDwOrderNumber() == $order->order_no) {
            echo "\nOrdine già caricato: ".$order->order_no;
        } else {
            $helper = new MagentoOrderHelper();
            //27/10/2016
            if ($order->isAuthenticated=='true')
                $helper->setCustomerDWCode($order->customer, $this->file);
            //se l'utente è guest non lo devo creare in magento ma devo solo metterlo nell'ordine
            $helper->createOrder($order, $this->file);

            //get order id
            $increment_id = $helper->getOrderIdByDWId($order->order_no);
            //$helper->setStatusPending($increment_id);
            $helper->setStatusComplete($increment_id);
        }
...
//$t = new OrderXMLAnalyzer('/Users/vincenzosambucaro/PhpstormProjects/OrderManagement/testFiles/order_export/inbound/20150306201139-order_cc_it_DW_SG_20150306191001.xml');
...
echo "\nStarting: ";
//$t = new OrderXMLAnalyzer('/home/OrderManagement/testFiles/order_export/archive/Export_2016_10_26.xml');
//$t = new OrderXMLAnalyzer('/home/OrderManagement/testFiles/order_export/archive/order_ov_it_DW_SG_20160930093001.xml');
//$t = new OrderXMLAnalyzer('/home/OrderManagement/testFiles/order_export/archive/order_ov_it_DW_SG_20160919100501.xml');
$t = new OrderXMLAnalyzer('/home/OrderManagement/testFiles/order_export/archive/order_ov_it_DW_SG_20161001135501.xml');
$t->process();
//echo "\nFine: ";