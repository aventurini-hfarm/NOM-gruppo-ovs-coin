...
require_once '../omdb/ShipmentDBHelper.php';
require_once '../dw2om/orders/MagentoOrderHelper.php';
...
//    return $obj;
    //$lista = array('100000265','100000266'); //PER TEST
    foreach ($lista as $increment_id) {
        ...
        if ($order->getStatus()=='complete')
            if ( controlloValiditaDataReso($objShipment->delivery_date) )
                $obj->orderStatus="Spedito";
            else
                $obj->orderStatus="Chiuso";
        elseif  ($order->getStatus()=='canceled')
            $obj->orderStatus="Annullato";
        else
...
function GetOrderLinesByWebUserId($param) {
    ...
    $log->LogDebug("Getting Details: ".$increment_id." status : ".$obj->orderStatus);
    ...
    foreach ($lines as $line) {
        ...
        if ($obj->orderStatus=="In lavorazione")
            $lineObj->enableRefundFlag=false;
        else
            if ( controlloValiditaDataReso($objShipment->delivery_date) )
                $lineObj->enableRefundFlag=true;
            else
                $lineObj->enableRefundFlag=false;


        //$log->LogDebug("SKU Refund Flag: ".$lineObj->enableRefundFlag);
...
function controlloValiditaDataReso($delivery_date){
    $gg = 30; //giorni disponibili per effettuare il reso
    $NOW = date('Y-m-d');
    $diff = ( (strtotime($NOW) - strtotime($delivery_date) ) / 3600 ) / 24;
